...
## 顧客とクラウドプロバイダ間のSLOの壁を打破する

顧客とクラウドプロバイダの懸念の間にある事実上の壁は、自然である、または避けられないように思われるかもしれません。
GoogleにはEvernoteを実行しているGCPプラットフォームのSLOとSLA(サービスレベル契約)がありますが、Evernoteには独自のSLOとSLAがあります。
そのような2つのエンジニアリングチームが互いのSLAについて知らされるとは必ずしも期待されていません。

Evernoteはそのような壁を望んでいませんでした。
もちろん、SLOとSLAを基になるGCPメトリクスに基づいて設計して壁を作って分け隔てることもできます。
そうはしないで、はじめに、どのパフォーマンス特性が最も重要であるか、またその理由をGoogleに理解して欲しいと考えました。
私たちはGoogleの目的を私たちの目的と一致させ、両社がEvernoteの信頼性の成功と失敗を共通の責任と見なすことを望んでいました。
これを実現するには、以下の方法が必要でした。

* 目的を合わせる
* パートナー(この場合はGoogle)が、私たちにとって重要なことを本当に理解していることを確認する
* 成功と失敗の両方を共有する

ほとんどのサービスプロバイダは、クラウドサービスのために公開されているSLO/SLAを管理しています。
このコンテキスト内で作業することは重要ですが、クラウドプロバイダの環境内で当社のサービスがどの程度うまく機能しているかを総合的に表すことはできません。

例えば、特定のクラウドプロバイダーは、おそらく世界中で何十万もの仮想マシンを実行しており、それらは稼働時間と可用性のために管理されています。
GCPは、Compute Engine(つまりその仮想マシン)に99.95％の可用性を約束します。
GCPのSLOグラフが緑色(つまり99.95％超)であっても、同じSLOに対するEvernoteの見解は大きく異なる可能性があります。
仮想マシンの設置面積はGCP全体数のほんのわずかな割合であるため、我々の領域に分離された(または他の理由で分離された)機能停止は、グローバルレベルへの全体的な集計で「失われる」ことがあります。

このようなシナリオを修正するために、GoogleとSLOおよびSLOに対するリアルタイムのパフォーマンスを共有します。
その結果、Google CREチームとEvernoteの両方が同じパフォーマンスダッシュボードで作業します。
これは非常に単純な点のように思えるかもしれませんが、真の顧客志向の行動を促進するためのかなり強力な方法であることが判明しています。
その結果、一般的な「Service Xの実行速度が遅い」タイプの通知を受信するのではなく、Googleは私たちの環境に固有の通知を提供してくれます。
例えば、一般的な「今日のGCP負荷分散環境は遅い」に加えて、この問題がEvernoteのSLOに5％の影響を及ぼしていることも知らされます。
この関係は、Googleのチームにとっても役立ちます。チームは、自分の行動や決定が顧客にどのような影響を与えるのかを確認できます。

この双方向の関係性は大きなインシデントをサポートするための非常に効果的な枠組みも与えてくれました。
ほとんどの場合、通常のP1からP5のチケットとレギュラーサポートチャネルのモデルがうまく機能し、良好なサービスとGoogleとの良好な関係を維持することができます。
しかし、P1チケット(「ビジネスへの大きな影響」)だけでは不十分な場合があります。つまり、サービス全体が危険にさらされて、ビジネスに大きな影響が及ぶ場合があります。

このような時には、SLOの共有とCREチームとの関係が実を結びます。
SLOの影響が十分に大きければ、両者は特別扱いのあるP1チケットとしてこの問題を対処することになるという共通の理解を得ています。
多くの場合、これはEvernoteとGoogleのCREチームが共有されたカンファレンスブリッジを急速に動員することを意味します。
Google CREチームは、私たちが共同で定義し合意したSLOを監視しているため、優先順位付けと適切な対応に関して同期を保つことができます。

## 現在の状況

約9か月間SLOを積極的に使用した後、Evernoteでは既にSLOプラクティスのバージョン3を採用しています。
次のバージョンでは、私たちの単純な稼働時間のSLOから進展するでしょう。
個々のAPIコールの調査を開始し、クライアント内のメトリックまたはパフォーマンス観点を考慮に入れることで、ユーザーのQoSをより良く表現することができます。

QoSを測定するための標準的で定義された方法を提供することで、SLOによってEvernoteではサービスがどのように稼働しているかについてより集中できるようになりました。
今では、機能停止の影響について、内部会議、Googleとの会議両方でデータ駆動の議論を持つことができます。これによってサービス改善を促進し、究極的にはより効果的なサポートチームとより幸せな顧客を作ることができるようになっています。
