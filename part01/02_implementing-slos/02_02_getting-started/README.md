...
SLIの設定を始める最も簡単な方法は、システムをいくつかの一般的な型のコンポーネントに抽象化することです。
次に、各コンポーネントに推奨されるSLIのリストを使用して、サービスに最も関連性のあるSLIを選択することができます。

_リクエスト駆動_  
ユーザは何らかの形式のイベントを作成し、レスポンスを期待します。
例えば、これはユーザがモバイルアプリケーションのブラウザまたはAPIとやりとりするHTTPサービスである可能性があります。

_パイプライン_  
レコードを入力として受け取り、それらを変更して他の場所にアウトプットを配置するシステム。
これは、リアルタイムに単一のインスタンス上で実行される単純なプロセス、あるいは何時間もかかる多段バッチ処理である可能性があります。
例として以下のものが含まれます。

* リレーショナルデータベースから定期的にデータを読み込み、最適な提供のために分散ハッシュテーブルに書き込むシステム
* ビデオを特定のフォーマットから別のフォーマットに変換するビデオ処理サービス
* 多量のソースからログファイルを読み込んでレポートを生成するシステム
* リモートサーバからメトリクスを取得し、時系列およびアラートを生成する監視システム

_ストレージ_  
データ(バイト、レコード、ファイル、ビデオなど)を受け取り、後で検索できるようにするシステム。
